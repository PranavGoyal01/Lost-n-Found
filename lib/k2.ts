type K2DateIdeaInput = {
	userAProfile: string;
	userBProfile: string;
	commonMeetingLocation: string;
...
export async function generateIdealDateFromK2(input: K2DateIdeaInput): Promise<string | null> {
	...
	const prompt = [
		"You are planning one concise first-date idea for two people who just matched.",
		"Use BOTH full profiles and the moment location data.",
		"The date should be realistic and near the provided common meeting area.",
		"Return only one sentence, no bullets, no labels.",
		`Person A profile: ${input.userAProfile}`,
		`Person B profile: ${input.userBProfile}`,
		`Common meeting location: ${input.commonMeetingLocation}`,
		`Location context from both moments: ${input.locationContext}`,
	].join("\n");
