  ...
  if (existing) {
    ...
    if (updated?.user_a_confirmed && updated?.user_b_confirmed) {
      ...
      const [userA, userB] = await Promise.all([
        getUserContact(updated.user_a_id),
        getUserContact(updated.user_b_id),
      ]);
      ...
      await Promise.all([
        notifyIfPhoneExists(
          userA?.phone_number ?? null,
          `Lost n Found: ${userBName} confirmed your moment match. You are now connected.`
        ),
        notifyIfPhoneExists(
          userB?.phone_number ?? null,
          `Lost n Found: ${userAName} confirmed your moment match. You are now connected.`
        ),
      ]);
    }
  } else {
    ...
    const { data: created } = await supabase.from('confirmations').insert([{
    ...
    if (created?.user_a_id && created?.user_b_id) {
      const [requester, recipient] = await Promise.all([
        getUserContact(created.user_a_id),
        getUserContact(created.user_b_id),
      ]);
