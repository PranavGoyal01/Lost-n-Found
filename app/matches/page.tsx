...
export default function Matches() {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				...
				const { data: pendingData } = await supabase.from("confirmations").select("*, moments_a:moment_a_id(description), moments_b:moment_b_id(description)").or(`user_a_id.eq.${session.user.id},user_b_id.eq.${session.user.id}`);
	...
	const acceptPendingMatch = async (currentPostId: string, otherPostId: string) => {
		...
		await fetch("/api/confirmations", { method: "POST", headers: { Authorization: `Bearer ${session?.access_token}` }, body: JSON.stringify({ current_post_id: currentPostId, other_post_id: otherPostId }) });
					...
					pending.map((p) => {
						...
						return (
							<div key={p.id} className='border p-4 mb-3 bg-yellow-50 rounded-lg shadow-sm'>
								<p className='text-sm font-semibold text-yellow-800 mb-2'>{waitingOnThem ? "⏳ Waiting for their response..." : "✨ They think you're a match!"}</p>
								...
								{!waitingOnThem && (
									<button onClick={() => acceptPendingMatch(myPostId, theirPostId)} className='mt-3 w-full bg-black text-white p-2 rounded font-semibold hover:bg-gray-800 transition'>
										Accept Match
									</button>
								)}
							</div>
				...
				{matches.length === 0 ? (
					...
					matches.map((m) => (
						<details key={m.id} className='border p-4 mb-3 bg-green-50 rounded-lg overflow-hidden shadow-sm'>
							<summary className='font-bold cursor-pointer text-green-800'>🎉 Match Found! Chat ID: {m.chat_id}</summary>
