...
function LocationPickerMap({
  ...
  useEffect(() => {
    ...
    mapRef.current.flyTo([center.lat, center.lng], Math.max(mapRef.current.getZoom(), 14), {
      duration: 0.5,
    });
  }, [center]);
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="Cafe, station, neighborhood..."
          />
          <button
            type="button"
            onClick={findLocation}
            disabled={locationLookupLoading}
            className="px-4 py-2 rounded bg-gray-900 text-white font-medium disabled:bg-gray-500"
          >
        ...
        {locationLabel ? (
          <p className="text-xs text-gray-500 leading-relaxed">{locationLabel}</p>
        ) : null}
      </div>

      <div className="space-y-2">
        <p className="text-sm text-gray-700 font-medium">
          Refine the exact point (drag pin, zoom, or tap map)
        </p>
        ...
        {selectedPoint ? (
          ...
          <p className="text-xs text-gray-500">
            Search a place first, then move the pin for exact location.
          </p>
        )}
      </div>
