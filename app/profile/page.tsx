...
export default function Profile() {
	...
	const handleSave = async () => {
		...
		try {
			const res = await fetch("/api/users", { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session?.access_token}` }, body: JSON.stringify(editForm) });
			if (res.ok) {
				setIsEditing(false);
				await fetchProfile();
			}
			...
			<main className='flex-1 flex flex-col items-center px-6 py-14'>
				<div className='w-full max-w-sm'>
					...
					<div className='text-center mb-10'>
						...
						<h1 className='text-[32px] font-normal tracking-tight text-gray-900 leading-tight' style={{ fontFamily: "Georgia, Times New Roman, serif" }}>
							Your profile
						</h1>
					</div>
					...
					<div className='flex justify-center mb-10'>
						<ProfilePictureUpload userId={profile.id} currentPictureUrl={profile.profile_picture || null} onUploadSuccess={(newUrl) => setProfile({ ...profile, profile_picture: newUrl })} />
					</div>
					...
					{isEditing ? (
						<div className='flex flex-col gap-4'>
							<div>
								...
								<input className={inputClass} value={editForm.name || ""} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} placeholder='Your name' />
							</div>
							<div>
								<label className={labelClass}>Phone</label>
								<input className={inputClass} value={editForm.phone_number || ""} onChange={(e) => setEditForm({ ...editForm, phone_number: e.target.value })} placeholder='+1 000 000 0000' />
							</div>
							<div>
							...
							<div>
								...
								<textarea className={textareaClass} value={editForm.likes || ""} onChange={(e) => setEditForm({ ...editForm, likes: e.target.value })} placeholder='Coffee, hikes, indie music, art museums...' />
							</div>
							<div>
								<label className={labelClass}>Dislikes</label>
								<textarea className={textareaClass} value={editForm.dislikes || ""} onChange={(e) => setEditForm({ ...editForm, dislikes: e.target.value })} placeholder='Crowded bars, spicy food, early mornings...' />
							</div>
							<div className='flex gap-3 mt-2'>
								<button onClick={handleSave} disabled={saving} className='flex-1 bg-gray-900 text-white text-[14px] font-medium py-3 rounded-lg hover:bg-gray-700 transition-colors disabled:opacity-40'>
									{saving ? "Saving…" : "Save"}
								</button>
								<button
									onClick={() => {
						...
						<div className='flex flex-col gap-6'>
							...
							{/* Actions */}
							<button onClick={() => setIsEditing(true)} className='w-full bg-gray-900 text-white text-[14px] font-medium py-3 rounded-lg hover:bg-gray-700 transition-colors'>
								Edit profile
							</button>
							<button onClick={handleSignOut} className='w-full text-[13px] text-gray-400 hover:text-red-500 transition-colors py-1'>
								Sign out
							</button>
						</div>
					)}
